...
> **2026-09-02 更新：P0 + P1 已全部落地并复测**，见「六、优化落地与复测对比」。
...
> 状态：**P0-1 / P0-2 / P1-1 / P1-2 / P1-3 已实施**（2026-09-02，见「六、优化落地与复测对比」）；以下保留原文供追溯，P2/P3 待做。

### P0-1 应用多副本横向扩展【已实施 ✔】（预期 ×2.5-3，改动最小收益最大）
...
### P0-2 requireUser 封禁检查加短缓存【已实施 ✔】（省 1 次 PG 往返/每个登录态请求）
...
### P1-1 roadmaps / announcements 列表加缓存【已实施 ✔】（照抄 works:list 模式）
...
## 六、优化落地与复测对比（2026-09-02，V4.1）

已实施：P0-1（app × 3 副本 + Caddy `dynamic a` 动态上游负载均衡）、P0-2（requireUser 状态缓存 30s + 封/解封主动失效）、P1-1（roadmaps 列表 60s / 详情公共部分 300s、announcements 公共列表 60s）、P1-2（搜索 10s 缓存 + trgm 索引补全）、P1-3（/auth/me 30s 聚合缓存，全链路主动失效）。

同矩阵同参数复测（服务器端，经 Caddy，压测文件 `*-after.json`）：

| 指标                    | 优化前                | 优化后                               | 提升     |
| ----------------------- | --------------------- | ------------------------------------ | -------- |
| 读容量 vu=50            | 125.3 RPS             | **415.4 RPS**                        | **3.3×** |
| 读容量 vu=100           | 107.2 RPS（吞吐倒挂） | **472.5 RPS**（继续上升）            | **4.4×** |
| 混合负载 vu=50          | 93.5 RPS              | **215.3 RPS**                        | **2.3×** |
| 并发登录 p50（10 并发） | 4390ms                | **1724ms**                           | **2.5×** |
| 读场景 p50（vu=50）     | 219-752ms             | **82-157ms**                         | 3-7×     |
| 错误                    | 0                     | 0（仅登录专项 3/130 瞬时 502，见下） | —        |

资源画像变化：单副本 0.7 核饱和 → 三副本各 ~0.5 核（总 CPU 的 ~13%/副本），PostgreSQL/Redis 仍 <5%。**瓶颈已从「应用进程数」转移开**；当前 472 RPS 时 4 核仍有余量，下一瓶颈预计在带宽或 PG 连接（均未到）。

实施中发现并修正的两个问题：

1. **Caddy 静态上游会钉死单容器**：Docker DNS 对 `app` 返回全部 3 条 A 记录，但 `reverse_proxy app:3000` 在长连接复用下把流量钉在一个副本上（实测 90 请求 0/0/30KB 分布）。改为 `dynamic a` 动态上游后均匀分布（11/11/8KB）。**多副本部署必须用 dynamic a**。
2. **登录专项出现 3 个瞬时 502**（130 次登录中，2.3%）：推测为 dynamic a 地址池刷新时旧连接被回收的瞬时窗口。低频且自愈；若需消除可给 Caddy 配 `health_check` + 活跃探测重试，当前规模不值得加复杂度。

缓存正确性专项验证（`scripts/smoke-cache.sh`，线上 10/10）：封禁后旧会话**立即** 403（无 30s 延迟）、解封立即恢复、公告发布后公共列表立即可见、read-all 后未读红点立即清零——所有「主动失效」链路在真实链路生效，缓存对用户不可感知。

P1-3 实现说明：未读计数没有采用报告原稿的「写时增减计数器」方案（公告是广播写，无法按用户 INCR），改为**整聚合 30s 缓存 + 全部写路径主动失效**，语义更稳（无计数漂移风险），效果相同（每请求 3 查询 → 缓存命中 0 查询）。

---

# V8 全量复测（2026-09-05，上真实数据前验收）

> 环境：V7 免费模式 + V8 荣耀引擎全部上线后（app ×3 副本 512M，main=58d9e2c+）。
> 工具：`scripts/stress.mjs` 扩展版（新增 V8 场景 userHonor/myHonor/achPop/myFavorites/download/rating/pin/featured；修复 userProfile 打 /users/u0 恒 404 的旧 bug）。
> 数据：120 压测账号（每人预授 HELP_10/HELP_50 两枚成就，SQL 复制 stress001 哈希建号）× 29 作品 × 3 路线图。原始数据 `scripts/stress-results/V8-*.json`。

## 结论

| 系列   | 场景                              | VU/时长   | 吞吐                         | 错误            | 关键延迟                                     |
| ------ | --------------------------------- | --------- | ---------------------------- | --------------- | -------------------------------------------- |
| A1     | 公开读（本机经 Caddy）            | 100/90s   | 406.9 RPS                    | 0               | p50 156-442ms                                |
| B1     | 混合负载                          | 100/120s  | 187.3 RPS                    | 0.01%（2×502）  | 写路径 p50 572-1052ms                        |
| C1     | 写专项                            | 60/90s    | 124.6 RPS                    | 0               | rating 201×293/409×912（唯一约束，业务语义） |
| D1     | 登录风暴（修复前 bcryptjs）       | 100/120s  | 5.8 login/s                  | 0.88%           | **p50 12.8s / p99 58.3s / max 64.4s** ⚠      |
| D2     | 登录风暴（换 @node-rs/bcrypt 后） | 100/120s  | **10.9 login/s**             | 0.07%           | **p50 6.7s / p99 15.1s / max 15.7s**         |
| 双进程 | 登录风暴 ∥ 读流量                 | 50+50/60s | 读 281 RPS                   | 0               | **读 p50 120ms——登录风暴不再拖死全站**       |
| E1     | V8 荣耀专项                       | 60/90s    | 140.9 RPS                    | 0               | achPop/myHonor p50 295-422ms                 |
| S1     | 浸泡（混合持续负载）              | 100/600s  | **211.7 RPS / 127,103 请求** | 0.003%（4×502） | 内存 53→77M 暖机后走平，无泄漏               |
| F1     | 公网用户视角（本机→kedahub.cn）   | 100/120s  | 198.6 RPS                    | 0               | p50 264-972ms（含 ~180ms RTT）               |

百人并发目标达成：读/混合/写/荣耀/公网全场景 0-0.01% 错误、无 5xx 雪崩、无 OOM、无连接池耗尽、10 分钟浸泡内存平稳。

## 修复：登录风暴饿死全站（本轮唯一发现的实质缺陷）

- **根因**：`bcryptjs` 是纯 JS 实现，跑在 event loop 上。100 并发登录时 bcrypt 完全占满 event loop，**同进程的所有其他请求（含健康检查）排队 60s+**。
- **修复**：换 `@node-rs/bcrypt`（Rust napi 原生，跑 libuv 线程池）。hash 格式 $2a/$2b 双向兼容，存量哈希无需迁移（已验证 demo/stress 全部存量账号登录正常）。webpack 需外部化：`serverComponentsExternalPackages` 加 `@node-rs/bcrypt`。
- **效果**：登录吞吐 +88%（5.8→10.9/s）、p99 -74%（58.3→15.1s）；并行验证（登录风暴进行时打读流量 281 RPS、p50 120ms、0 错误）确认隔离成立。
- 登录持续容量 ~11/s ≈ 每 5 分钟 3300 人登录；登录尖峰的剩余表现为 CPU 排队变慢（不失败、不扩散），校园规模下不再需要进一步优化。

## 容量画像（4C/3.7G 单机）

- 应用层仍是第一瓶颈（3 副本 CPU ~110% 时 PG 仅 18-25%、Redis <6%、PgBouncer ~0%）——数据库与缓存余量巨大，扩 app 副本或升配即可线性抬升。
- 重载下无失败、表现为排队（p99 2-3s）；瞬时 502 共 7 个/15 万请求（0.005%，Caddy dynamic a 地址池刷新的已知瞬时窗口，自愈）。
